...
namespace fractal {
	namespace fcore {
		...
		void CameraManager::setActiveCamera(const std::string& name) {
			...
			std::vector<fscene::CameraObject*>::iterator it = std::find_if(this->m_cameras.begin(), this->m_cameras.end(),
				[name](fscene::CameraObject* camera)
			{
		...
		void CameraManager::setActiveCamera(fscene::CameraObject* camera) {
			std::vector<fscene::CameraObject*>::iterator it = std::find(this->m_cameras.begin(), this->m_cameras.end(), camera);
			if (it == this->m_cameras.end()) {
		...
		fscene::CameraObject* CameraManager::getActiveCamera() const {
			return m_activeCamera;
		}

		void CameraManager::addCamera(fscene::CameraObject* camera, bool setActive) {
